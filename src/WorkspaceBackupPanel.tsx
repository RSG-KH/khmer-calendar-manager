...
export function WorkspaceBackupPanel({ snapshot, dirty, busy, perform, restored, download, error }: Props) {
  ...
  return <section className="panel padded">
    ...
    {backup && <div className="notice info"><div><h3>Review workspace restoration</h3><p>Revision {backup.workspace.revision} · {backup.workspace.data.sources.length} sources · {backup.workspace.data.events.length} rules/events · {backup.workspace.data.eventCalendars.length} recorded years · {backup.workspace.data.holidayCalendars.length} holiday years · {changes(snapshot.workspace.data, backup.workspace.data).length} catalog changes.</p>
